...
const exec = (parser, args, cb) => {
  ...
  try {
    ...
    // if the server mock function just returns a value instead of calling callback,
    // then wrap it into a callback here
    if (cb.length === 1) {
      const result = cb(parsed); // eslint-disable-line callback-return
...
RedisClient.prototype.setEx = RedisClient.prototype.SETEX = function (key, value, seconds, callback) {
  this._selectedDb.set(key, value, () => {
    this._selectedDb.expire(key, seconds, (err, result) => {
      helpers.callCallback(callback, err, "OK");
    });
...
RedisClient.prototype.script = RedisClient.prototype.SCRIPT = function (...userArgs) {
  exec(parsers.script, userArgs, (args) => {
    const scriptArgs = args.multiple.map((arg) => arg.scriptArgs);
    return this._selectedDb.script(scriptArgs[0], ...scriptArgs.slice(1));
  });
};
